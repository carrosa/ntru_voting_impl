...
void bdlop_sample_chal(params::poly_q & f) {
    ...
    // The `nfl::hwt_dist {NTRU_NONZERO}` likely samples a polynomial with a fixed number of non-zero coefficients,
    // where the number is specified by the `NTRU_NONZERO` constant.
...
int bdlop_open(commit_t & com, vector < params::poly_q > m, comkey_t & key,
    ...
    if (_c1 != c1 || com.c2.size() != m.size()) {
        ...
        for (size_t i = 0; i < r.size(); i++) {
            ...
            if (!bdlop_test_norm(c1, 16 * NTRU_SIGMA_C)) {
